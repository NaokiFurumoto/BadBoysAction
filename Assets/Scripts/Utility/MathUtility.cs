...
public static class MathUtility
{
    ...
    /// </summary>
    /// ***********************************************************************
    public static int CeilPercentOf(int percentage, int value)
    {
        var v = percentage * value;
        var mod = v % 100;
        return (v / 100) + (v > 0 ? (mod == 0 ? 0 : 1) : 0);
    }
    ...
    /// </summary>
    /// ***********************************************************************
    public static ulong CeilPercentOf(int percentage, ulong value)
    {
        if (percentage < 0)
            Utility.Throw(new ArgumentException($"percentage:{percentage} < 0"));

        var v = (ulong)percentage * value;
        var mod = v % 100;
        return (v / 100) + (mod == 0 ? 0UL : 1UL);
    }
    ...
    /// <returns>回転角の差</returns>
    /// ***********************************************************************
    public static float CalcEulerDiff(float rot, float target)
    {
        // 0 - 360 に収める
    ...
    /// 角度幅を計算する(オイラー角)
    /// </summary>
    /// <returns>正の値で角度幅を返す</returns>
    /// ***********************************************************************
    public static float CalcRotRangeEuler(float a, float b)
    {
        a = MathUtility.Repeat(a, 0, 360.0f);
        b = MathUtility.Repeat(b, 0, 360.0f);
